...
checked.  First, select 2*SCALE vertices similarly to [[#sampleroot][sampling the root
vertices]].  Instead of using PRNG(NE, k), use PRNG(ceil(NE /
/kerneltime/), k) where /kerneltime/ is the time required by the kernel
being verified.  This produces a less predictable sequence of vertices.
For each such vertex, only the lesser of edgefactor and the vertex's
degree adjacent edges need to be checked.  This specification does not
