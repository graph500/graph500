...
full scale, the data set is enormous, and its exact details depend on on
the pseudo-random number generator and the BFS or SSSP algorithm used.
Therefore, the validation of an implementation of the benchmark uses soft
checking of the results.  Validation is *not* part of the timed results.

The executable specification verifies its results by comparing them
with results computed directly from the tuple list.  Note that the
SSSP kernel uses the sum of the weights for a particular edge that may
have multiple entries in the input tuple list.  Because the edges need
re-collapsed into a graph form, only a sampling of the edges are
checked.  Every tree edge must be checked.  Additionally, select
2*SCALE vertices similarly to [[#sampleroot][sampling the root vertices]].  For each
such vertex, only the lesser of edgefactor and the vertex's degree
adjacent edges need to be checked.  This specification does not
require any particular adjacent edges.
...
Algorithm [[alg:verify]] shows a sample validation routine.  This
