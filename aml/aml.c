...
#define AGGR \
  (1024 * 32)  // aggregation buffer size per dest in bytes : internode
#define AGGR_intra \
...
// MPI stuff for sends
static char *sendbuf;  // coalescing buffers, most of memory is allocated is
                       // here
...
static ushort
    activebuf[NSEND];  // N_buffer used in transfer(0..NSEND{_intra}-1)
...
inline void aml_send_intra(void *srcaddr, int type, int length, int local,
                           int from);
...
inline void flush_buffer(int node) {
  ...
  MPI_Isend(SENDSOURCE(node), sendsize[node], MPI_CHAR, node, acks[node], comm,
            rqsend + index);
...
SOATTR int aml_init(int *argc, char ***argv) {
  ...
  if ((group_size & groupmask)) {
    printf(
        "AML: Fatal: non power2 groupsize unsupported. Define macro "
  ...
  if (myproc == 0)
    printf("NRECV=%d NRECVi=%d NSEND=%d  NSENDi=%d AGGR=%dK AGGRi=%dK\n", NRECV,
           NRECV_intra, NSEND, NSEND_intra, AGGR >> 10, AGGR_intra >> 10);
#endif
  ...
  for (i = 0; i < NRECV_intra; i++) MPI_Start(rqrecv_intra + i);
  ...
  for (i = 0; i < NRECV; i++) MPI_Start(rqrecv + i);
  for (j = 0; j < NSEND; j++) {
...
SOATTR void aml_finalize(void) {
  int i;
  aml_barrier();
  for (i = 0; i < NRECV; i++) MPI_Cancel(rqrecv + i);
#ifndef NOINTRA
  for (i = 0; i < NRECV_intra; i++) MPI_Cancel(rqrecv_intra + i);
