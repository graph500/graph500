...
#pragma omp parallel for
    for (i = 0; i < nlocalverts; ++i) write_pred_entry_depth(&pred[i], UINT16_MAX);
  ...
  gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), MPI_INT64_T);
...
#pragma omp parallel for
        ...
        get_vertex_distribution_for_pred(i_end - ii, pred_vtx, pred_owner, pred_local);
#pragma omp parallel for
        for (i = ii; i < i_end; ++i) {
          if (pred[i] != -1) {
            add_gather_request(pred_win, i - ii, pred_owner[i - ii], pred_local[i - ii], i - ii);
          } else {
            pred_pred[i - ii] = -1;
          }
        }
...
#pragma omp parallel for reduction(&&:validation_passed) reduction(||:any_changes)
        for (i = ii; i < i_end; ++i) {
          ...
          if (get_depth_from_pred_entry(pred_pred[i - ii]) != UINT16_MAX) {
            if (get_depth_from_pred_entry(pred[i]) != UINT16_MAX && get_depth_from_pred_entry(pred[i]) != get_depth_from_pred_entry(pred_pred[i - ii]) + 1) {
              fprintf(stderr, "%d: Validation error: BFS predecessors do not form a tree; see vertices %" PRId64 " (depth %" PRIu16 ") and %" PRId64 " (depth %" PRIu16 ").\n", rank, vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]), get_pred_from_pred_entry(pred[i]), get_depth_from_pred_entry(pred_pred[i - ii]));
              validation_passed = 0;
            } else if (get_depth_from_pred_entry(pred[i]) == get_depth_from_pred_entry(pred_pred[i - ii]) + 1) {
              /* Nothing to do */
            } else {
              write_pred_entry_depth(&pred[i], get_depth_from_pred_entry(pred_pred[i - ii]) + 1);
              any_changes = 1;
            }
          }
        }
...
static int check_bfs_depth_map_using_predecessors(const int64_t nglobalverts, const size_t nlocalverts, const size_t maxlocalverts, const int64_t root, const int64_t* const pred) {
  int validation_passed = 1;
  int root_owner;
  size_t root_local;
  get_vertex_distribution_for_pred(1, &root, &root_owner, &root_local);
...
#pragma omp parallel for reduction(&&:validation_passed)
    for (i = ii; i < i_end; ++i) {
      ...
      if (get_depth_from_pred_entry(pred[i]) != get_depth_from_pred_entry(pred_pred[i - ii]) + 1) {
        fprintf(stderr, "%d: Validation error: BFS predecessors do not form a tree; see vertices %" PRId64 " (depth %" PRIu16 ") and %" PRId64 " (depth %" PRIu16 ").\n", rank, vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]), get_pred_from_pred_entry(pred[i]), get_depth_from_pred_entry(pred_pred[i - ii]));
        validation_passed = 0;
      }
    }
...
#pragma omp parallel for reduction(&&:validation_passed)
  ...
  if (bfs_writes_depth_map()) {
    int check_ok = check_bfs_depth_map_using_predecessors(nglobalverts, nlocalverts, maxlocalverts, root, pred);
...
#pragma omp parallel for reduction(&&:validation_passed) reduction(+:edge_visit_count)
        for (i = ii; i < i_end; ++i) {
          ...
          uint16_t src_depth = get_depth_from_pred_entry(edge_preds[(i - ii) * 2 + 0]);
          uint16_t tgt_depth = get_depth_from_pred_entry(edge_preds[(i - ii) * 2 + 1]);
          ...
          if (get_pred_from_pred_entry(edge_preds[(i - ii) * 2 + 0]) == tgt) {
            add_scatter_constant_request(pred_valid_win, edge_owner[(i - ii) * 2 + 0], edge_local[(i - ii) * 2 + 0], (i - ii) * 2 + 0);
          }
          if (get_pred_from_pred_entry(edge_preds[(i - ii) * 2 + 1]) == src) {
