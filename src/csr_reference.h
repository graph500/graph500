...
typedef struct oned_csr_graph {
  ...
  int64_t* column;
#ifdef SSSP
  float* weights;  // corresponed to column
#endif
...
#define SETCOLUMN(a, b) \
  memcpy(((char*)column) + (BYTES_PER_VERTEX * a), &b, BYTES_PER_VERTEX)
#define COLUMN(i)                                          \
  (*(int64_t*)(((char*)column) + (BYTES_PER_VERTEX * i)) & \
   (int64_t)(0xffffffffffffffffULL >> (64 - 8 * BYTES_PER_VERTEX)))
