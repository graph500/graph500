...
void frompredhndl(int from, void* data, int sz) {
  ...
  if (globpred[vloc] == predfrom && globdist[vloc] == FLT_MAX)
    globdist[vloc] = prevlevel + 1.0, newvisits++;
}
...
void vhalfedgehndl(int from, void* data, int sz) { vdegrees[*(int*)data]++; }
...
void sendedgepreddist(unsigned vloc, unsigned int vedge) {
  edgedist m = {vloc, VERTEX_LOCAL(COLUMN(vedge)), globpred[vloc],
                globdist[vloc]
#ifdef SSSP
                ,
                vweights[vedge]
...
#define DUMPERROR(text)                                                    \
  {                                                                        \
    printf(                                                                \
        "Validation Error: %s, edge %llu %llu weight %f pred0 %llu pred1 " \
        "%llu dist0 %f dist1 %f\n",                                        \
        text, v0, v1, w, predv0, predv1, distv0, distv1);                  \
    val_errors++;                                                          \
    return;                                                                \
  }

// main validation handler: tracks all edges and at delivery has both vertex
// preds and distances to be checked
void edgepreddisthndl(int frompe, void* data, int sz) {
  ...
  if (predv0 == -1 && predv1 == -1)
    return;
  else if (v0 < v1)
    nedges_traversed++;

  if ((predv0 == -1 && predv1 != -1) || (predv1 == -1 && predv0 != -1))
    DUMPERROR("edge connecting visited and unvisited vertices");
  if (predv1 == v0 && distv1 == distv0 + w)
    confirmed[v1loc] = 1;  // confirm pred/dist as existing edge

  if (distv0 + w < distv1 || distv1 + w < distv0)
    DUMPERROR("triangle rule violated");
}
...
int validate_result(int isbfs, const tuple_graph* const tg,
                    ...
                    int64_t* const pred, float* dist, int64_t* nedges_in) {
  validatingbfs = isbfs;
  ...
  if (firstvalidationrun) {
    ...
    // Second pass , actual data transfer: placing edges to its places in
    // vcolumn
    ITERATE_TUPLE_GRAPH_BEGIN(tg, buf, bufsize, wbuf) {
      ptrdiff_t j;
      for (j = 0; j < bufsize; ++j) {
  ...
  for (size_t i = 0; i < nlocalverts; ++i)
    if ((pred[i] != -1 && pred[i] < 0) || pred[i] > maxvertex)
      printf(
          ...
          pred[i], VERTEX_TO_GLOBAL(my_pe(), i)),
          val_errors++;
  ...
  for (size_t i = 0; i < nlocalverts; ++i) {
    if (dist[i] != -1.0 && dist[i] < 0.0)
      printf(
          ...
          "range\n",
          dist[i], VERTEX_TO_GLOBAL(my_pe(), i)),
          val_errors++;
    if ((pred[i] == -1 && dist[i] != -1.0) ||
        (pred[i] != -1 && dist[i] == -1.0))
      printf(
          "Validation Error: vertex %llu has inconsistent predecessor %llu and "
          "distance %f\n",
          VERTEX_TO_GLOBAL(my_pe(), i), pred[i], dist[i]),
          val_errors++;
  }
  ...
  if (my_pe() == VERTEX_OWNER(root)) {
    ...
    if (pred[vloc] != root || dist[vloc] != 0.0)
      printf(
          "Validation Error: root vertex %llu has predecessor %llu and "
          "distance %f\n",
          root, pred[vloc], dist[vloc]),
          val_errors++;
    else
      confirmed[vloc] = 1;
  }
  ...
  for (size_t i = 0; i < nlocalverts; ++i)
    if (confirmed[i] == 0 && pred[i] != -1)
      printf(
          ...
          "predecessor %llu with distance %f\n",
          VERTEX_TO_GLOBAL(my_pe(), i), pred[i], dist[i]),
          val_errors++;
  ...
  if (nedges_in != NULL && *nedges_in != nedges_traversed)
    printf(
        "Validation Error: wrong nedge_traversed %llu (correct number is "
        "%llu)\n",
        *nedges_in, nedges_traversed),
        val_errors++;
  if (val_errors > 0)
    return 0;
  else
